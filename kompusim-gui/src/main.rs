...
use iced::widget::{
    button, column, container, horizontal_space, responsive, row, scrollable, text, tooltip,
};
use iced::{keyboard, Font};
use iced::{Color, Element, Length, Size, Subscription};

pub fn main() -> iced::Result {
    iced::application("Kompusim", KompusimGui::update, KompusimGui::view)
        .subscription(KompusimGui::subscription)
...
impl KompusimGui {
    ...
    fn update(&mut self, message: Message) {
        match message {
            Message::OpenDemo => {
                println!("DBG: open demo")
            }
            Message::SimRun => {
                println!("DBG: sim run")
            }
            Message::SimStep => {
                println!("DBG: sim step")
            }
    ...
    fn view(&self) -> Element<Message> {
        println!("DBG: view()");
        let controls_panel = row![
            action_btn(load_demo_icon(), "Load Demo", Some(Message::OpenDemo)),
            action_btn(open_icon(), "Sim Step", Some(Message::SimStep)),
            action_btn(step_icon(), "Sim Run", Some(Message::SimRun)),
            horizontal_space(),
        ]
        .spacing(10)
        ...
        column![controls_panel, pane_grid]
            .spacing(10)
            .padding(10)
            .into()
        // container(pane_grid)
...
fn action_btn<'a, M: Clone + 'a>(
    content: impl Into<Element<'a, M>>,
    label: &'a str,
    msg_on_press: Option<M>,
) -> Element<'a, M> {
    let btn = button(container(content).center_x(20));

    if let Some(on_press) = msg_on_press {
        tooltip(
...
// generated font with icons on https://fontello.com/
// gnome-font-viewer to check the font
fn load_demo_icon<'a, M>() -> Element<'a, M> {
    // icon('\u{e800}')
    icon('\u{f1c0}')
}

fn step_icon<'a, Message>() -> Element<'a, Message> {
    icon('\u{e801}')
    // icon('\u{e802}')
}
